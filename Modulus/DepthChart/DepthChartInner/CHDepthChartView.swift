...
open class CHDepthChartView: UIView {
    ...
    var mySuperView : INDepthChartView?
    ...
    fileprivate func resetData() {
        ...
        if plotCount > 0 {
            guard let item = self.delegate?.depthChart(chart: self, valueForPointAtIndex: 0) else {
                return
            }
            self.lowPrice = item.value
            self.heightPrice = item.value
            //获取代理上的数据源
            for i in 0...self.plotCount - 1 {
                ...
                // 计算最大最小值
                if item.value > self.heightPrice{
                    self.heightPrice = item.value
                }
                
                if item.value < self.lowPrice{
                    self.lowPrice = item.value
                }
                switch item.type {
                case .bid:
            ...
            self.priceRange = self.heightPrice - self.lowPrice
            //计算深度数量
...
extension CHDepthChartView {
    ...
    func drawLayerView() {
        ...
        var yAxisToDraw = self.drawYAxis()
        
        var filteredYaxis  = yAxisToDraw.sorted { (first, secound) -> Bool in
             first.0.origin.y > secound.0.origin.y
        }
        if filteredYaxis.count > 0 {
            filteredYaxis.removeFirst()
        }
        yAxisToDraw = filteredYaxis
...
//                let yLabelRect2 = CGRect(x: self.bounds.width - yAxisLabelWidth,
//                                         y: startY,
//                                         width: yAxisLabelWidth,
//                                         height: 12)
                
                yAxisToDraw.append((yLabelRect, strValue))
//                 yAxisToDraw.append((yLabelRect2, strValue))
    ...
    fileprivate func drawYAxisLabel(_ yAxisToDraw: [(CGRect, String)]) {
        ...
        for (yLabelRect, strValue) in yAxisToDraw {
            ...
            mySuperView?.leftTextLayer = yAxisLabel
            
            let rightSideRect = CGRect(x: self.bounds.width - yLabelRect.size.width - yLabelRect.origin.x, y: yLabelRect.origin.y, width: yLabelRect.size.width, height: yLabelRect.size.height)
            let yAxisLabelRight = CHTextLayer()
            yAxisLabelRight.frame = rightSideRect
            yAxisLabelRight.string = strValue
            yAxisLabelRight.fontSize = self.labelFont.pointSize
            yAxisLabelRight.foregroundColor =  self.textColor.cgColor
            yAxisLabelRight.backgroundColor = UIColor.clear.cgColor
            yAxisLabelRight.alignmentMode = CATextLayerAlignmentMode.right
            yAxisLabelRight.contentsScale = UIScreen.main.scale
            
            self.drawLayer.addSublayer(yAxisLabelRight)
            mySuperView?.leftTextLayer = yAxisLabelRight
            
            let line = CAShapeLayer()
            let linePath = UIBezierPath()
            linePath.move(to: CGPoint(x: 0, y: yLabelRect.origin.y))
            linePath.addLine(to: CGPoint(x: rightSideRect.origin.x + rightSideRect.size.width + yLabelRect.origin.x, y: rightSideRect.origin.y))
            line.path = linePath.cgPath
            line.strokeColor = UIColor.white.withAlphaComponent(0.5).cgColor
            line.lineWidth = 0.3
            line.lineJoin = CAShapeLayerLineJoin.round
            self.drawLayer.addSublayer(line)
    ...
    fileprivate func drawXAxisLabel(xAxisToDraw: [(CGRect, String)]) {
        ...
        for (index,(var barLabelRect, xLabel)) in xAxisToDraw.enumerated() {
            ...
            barLabelRect.origin.y = startY + 5
            //绘制文本
            let xLabelText = CHTextLayer()
            xLabelText.frame = barLabelRect
            var textToDisplay = xLabel
            if shouldFormatDoubles {
            if let xDouble = Double(textToDisplay){
                textToDisplay = xDouble.kmFormatted
            }
            }
            xLabelText.string = textToDisplay
    ...
    func drawItemInfo(point: CGPoint,item:CHKDepthChartItem){
        ...
        if let view = self.delegate?.depthChartShowItemView?(chart: self, Selected: item) {
            ...
            if item.type == .bid{
                typelayer.string = "Buy"
            }else{
                typelayer.string = "Sell"
            }
            ...
            pricelayer.string =  String(format: "%.6f", Double((item.value))) //  "Price: \(item.value.ch_toString(maxF:self.decimal))"//String(Double(iteme.value))
            ...
            vollayer.string = "Volume: \(amountStr)"
            textRect = CGRect(x: textRect.origin.x, y: textRect.origin.y + textHeight, width: width - padding, height: textHeight)
    ...
    //MARK:- Helper function check if lies in between
    func isFirstRectInside(firstRect:CGRect,axisELementsArray:[(CGRect,String)])->Bool{


        var shouldReturn = false
        if firstRect.origin.x < 0{
            shouldReturn = true
            return shouldReturn
        }
        let sorted = axisELementsArray.sorted { (first, secound) -> Bool in
            first.0.origin.x < secound.0.origin.x
        }
        for element in sorted{
            let secoundRect = element.0
            if( firstRect.origin.x > (secoundRect.origin.x+secoundRect.width) || (firstRect.origin.x+firstRect.width) < secoundRect.origin.x ) && (firstRect.origin.x+firstRect.width) < UIScreen.main.bounds.width{
                shouldReturn = false
            }else{
                shouldReturn = true
                break
    ...
    func drawChart() -> [(CGRect, String)]{
        ...
        var xAxisToDraw = [(CGRect,String)]()
        let yAxisLabelWidth:CGFloat = self.isInnerYAxis ? 0 : self.yAxisLabelWidth + 3
        let chartWidth = self.bounds.size.width - self.padding.left - yAxisLabelWidth
        guard self.plotCount > 0 else {
            return []
        }
        if self.bidChartOnDirection == .left{
            //买单价格坐标/值
            let bidsX = self.bounds.origin.x + self.padding.left
            let bidsY = self.bounds.origin.y + self.bounds.size.height
            let bidsRect = CGRect(x: bidsX, y: bidsY, width: 60, height: 18)
            if xAxisToDraw.count > 0 {
                if isFirstRectInside(firstRect: bidsRect, axisELementsArray:xAxisToDraw) == false{
                     if self.bidItems.count > 0{
            ...
            if self.leftX.isNaN == true{ self.leftX = (chartWidth/2) - 10.0}
            ...
            if xAxisToDraw.count > 0 {
                if isFirstRectInside(firstRect: buy1Rect, axisELementsArray:xAxisToDraw) == false{
                    if self.bidItems.count > 0{
            ...
            if let bidChartLayer = self.drawDepthChart(items: self.bidItems, startX: 0, plotWidth: self.bidPlotWidth, strokeColor: self.bidColor.stroke, fillColor: self.bidColor.fill, lineWidth: self.bidColor.lineWidth) {
                ...
                if let bidChartLayerBG = self.drawDepthChart(items: self.bidItems, startX: 0, plotWidth: self.bidPlotWidth, strokeColor: UIColor.green, fillColor: UIColor.clear, lineWidth: self.bidColor.lineWidth+0.5) {
                    self.drawLayer.insertSublayer(bidChartLayerBG, below: bidChartLayer)
                    //                startIndex = self.bidItems.count
                }
            }
            ...
            if xAxisToDraw.count > 0 {
                if isFirstRectInside(firstRect: sell1Rect, axisELementsArray:xAxisToDraw) == false{
                    if self.askItems.count > 0{
            ...
            if let askChartLayer = self.drawDepthChart(items: self.askItems, startX: self.rightX, plotWidth: self.askPlotWidth, strokeColor: self.askColor.stroke, fillColor: self.askColor.fill, lineWidth: self.askColor.lineWidth) {
                ...
                if let askChartLayerBG = self.drawDepthChart(items: self.askItems, startX: self.rightX, plotWidth: self.askPlotWidth, strokeColor: UIColor.red, fillColor: UIColor.clear, lineWidth: self.askColor.lineWidth+0.5) {
                    self.drawLayer.insertSublayer(askChartLayerBG, below: askChartLayer)
                    //                startIndex = self.bidItems.count
                }
            ...
            if xAxisToDraw.count > 0 {
                if isFirstRectInside(firstRect: asksRect, axisELementsArray:xAxisToDraw) == false{
                     if self.askItems.count > 0{
            ...
            //买单价格坐标/值
            let asksX = self.bounds.origin.x + self.padding.left
...
//            index = startIndex + i
            ...
            //开始X
            var ix = startX + self.bounds.origin.x + self.padding.left + CGFloat(i) * plotWidth
